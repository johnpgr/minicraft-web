...
import { unpackTint } from '../assets/palette';
import { CHUNK_SIZE, createChunkKey, getChunkCoords, getDirtyRegion, groupTilesByChunk, type ChunkData, type ChunkKey } from './chunks';
...
import type { LightInstance, RenderFrame, SpriteInstance, TileInstance, UiSpriteInstance } from './types';
...
export class Renderer {
    ...
    init(canvas: HTMLCanvasElement, assets: RendererAssets): void {
        ...
        this.uiCamera = new THREE.OrthographicCamera(0, window.innerWidth, window.innerHeight, 0, -100, 100);
        ...
        this.worldSpriteBatch = this.createBatch(MAX_WORLD_SPRITES, this.spriteMaterial, this.worldScene);
    ...
    render(state: GameState, frame: RenderFrame): void {
        if (!this.initialized || !this.renderer || !this.worldScene || !this.uiScene || !this.worldCamera || !this.uiCamera) {
            return;
        }
    ...
    private createBatch(maxInstances: number, material: THREE.ShaderMaterial, scene: THREE.Scene): DynamicBatch {
    ...
    private syncChunks(mapTiles: TileInstance[], dirtyTiles: Array<{ x: number; y: number }>): void {
    ...
    private rebuildChunk(chunk: ChunkData): void {
        ...
        const geometry = new THREE.PlaneGeometry(1, 1);
        const mesh = new THREE.InstancedMesh(geometry, this.tileMaterial.clone(), TILE_BATCH_CAPACITY);
        mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
        mesh.frustumCulled = false;
        mesh.renderOrder = 10;

        const frameAttr = new THREE.InstancedBufferAttribute(new Float32Array(TILE_BATCH_CAPACITY), 1);
        const flipAttr = new THREE.InstancedBufferAttribute(new Float32Array(TILE_BATCH_CAPACITY), 1);
        const tintAttr = new THREE.InstancedBufferAttribute(new Float32Array(TILE_BATCH_CAPACITY * 4), 4);
        const alphaAttr = new THREE.InstancedBufferAttribute(new Float32Array(TILE_BATCH_CAPACITY), 1);
        ...
        for (const tile of chunk.tiles) {
            ...
            const tint = unpackTint(tile.tintCode);
    ...
    private updateWorldSprites(sprites: SpriteInstance[]): void {
        ...
        this.writeBatch(this.worldSpriteBatch, sprites.length, (index) => {
            ...
            const tint = unpackTint(sprite.tintCode);
            return {
    ...
    private updateUiSprites(uiSprites: UiSpriteInstance[]): void {
        ...
        this.writeBatch(this.uiSpriteBatch, uiSprites.length, (index) => {
            ...
            this.scratchMatrix.compose(
                new THREE.Vector3(sprite.screenX + 4, this.viewportHeight - (sprite.screenY + 4), 0),
                new THREE.Quaternion(),
